...
## Testing Predictors
linear models
To further examine the relationship between attributes and songs on the Billboard, we decided to use logistic regression. We thought that it would be interesting to observe which attribute is the strongest predictor of a song becoming a #1 Billboard. To do this we decided to test with logistic regression how the attributes; danceability, energy, speechiness, valence, and tempo predicted how well a song would do in becoming #1 on the Billboard charts.

First, we will show how the attribute danceability impacts the weekly position of a song. 

```{r danceability linear-model}
combined %>%
  ggplot(aes(x = danceability, y = week_position)) + 
  geom_hex() 
...
combined %>%
  ggplot(aes(x = energy, y = week_position)) + 
  geom_hex() 
...
combined %>%
  ggplot(aes(x = speechiness, y = week_position)) + 
  geom_hex() 
...
combined %>%
  ggplot(aes(x = valence, y = week_position)) + 
  geom_hex() 
...
danceability, energy, speechiness, valence, and tempo

```{r r-squared-values}
glance(danceability_model)$adj.r.squared
glance(energy_model)$adj.r.squared
glance(speechiness_model)$adj.r.squared
glance(valence_model)$adj.r.squared
glance(tempo_model)$adj.r.squared
```
*Insert Discussion*
